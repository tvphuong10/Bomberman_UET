...
public class Room extends Object {
    ...
    public Room(int type, int level) {
        ...
        if (type == 0) {
            ...
            int r = ran.nextInt(4);
            if (r == 0) {
                s = Resources.Maps.get(3).str.clone();
            } else if (r == 1) {
                s = Resources.Maps.get(4).str.clone();
            } else if ( r == 2) {
                s = Resources.Maps.get(5).str.clone();
            } else {
                s = Resources.Maps.get(6).str.clone();
            }
        } else if (type == 2) {
            s = Resources.Maps.get(1).str.clone();
        } else {
            s = Resources.Maps.get(2).str.clone();
        }
        map = s;
    ...
    public int getBombStatus(int x, int y) {
        if (map[y][x].charAt(0) == 'B') {
